...
class Game:
    ...
    def start_text(self):
        ...
        text2 = font2.render('Python Project 2023', True, 'white')
        text2_rect = text2.get_rect(center=(s_width/2, s_height/2+60))
        screen.blit(text2, text2_rect)

        blink_font = pygame.font.SysFont('Calibri', 15)
        blink_text = blink_font.render('Klik ENTER Untuk Memulai Game', True, 'green')
        blink_text_rect = blink_text.get_rect(center=(s_width/2, s_height/2+120))

        # Perhitungan untuk membuat efek berkedip
        blink_interval = 500  # dalam milidetik
        current_time = pygame.time.get_ticks()

        # Jika selisih waktu lebih kecil dari separuh interval, gambar teksnya
        if current_time % (2 * blink_interval) < blink_interval:
            screen.blit(blink_text, blink_text_rect)

        blink_font = pygame.font.SysFont('Calibri', 15)
        blink_text = blink_font.render('Klik Escape Untuk Keluar Game', True, 'red')
        blink_text_rect = blink_text.get_rect(center=(s_width/2, s_height/2+150))

        # Perhitungan untuk membuat efek berkedip
        blink_interval = 700  # dalam milidetik
        current_time = pygame.time.get_ticks()
    ...
    def game_over_text(self):
        ...
        blink_font = pygame.font.SysFont('Calibri', 15)
        blink_text = blink_font.render('Klik ENTER Untuk Kembali Ke Menu Awal', True, 'green')
        blink_text_rect = blink_text.get_rect(center=(s_width/2, s_height/2+120))
    ...
    def create_lives(self):
        ...
        for i in range(self.lives):
            screen.blit(self.live_img, (10+n, s_height-880))#0+n -860
            n += 60
    ...
    def create_score(self):
        ...
        text_rect = text.get_rect(center=(s_width-75, s_height-870)) #-150, -850
        screen.blit(text, text_rect)
